...
def intersect(a, b):
    ...
    A key and value pair is included in the result only if the key exists in both given dictionaries.
...
class ParentsIncludedModelFormMixin(object):
    ...
    def _iterate_over_instances(self, method_name, *args, **kwargs):
        ...
        for instance, meta, base in zip(self.instances, self.metas, self.__class__.__bases__[1:]):
            # Temporary set values
            self.instance = instance
            self._meta = meta
            results.append(getattr(base, method_name)(self, *args, **kwargs))
    ...
    def clean(self):
        # We traverse in reverse order to keep in sync with get_declared_fields
        return reduce(intersect, reversed(self._iterate_over_instances('clean')))
